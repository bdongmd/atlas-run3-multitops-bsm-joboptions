...
class CondorHandler(object) :
  """
    ...
    send_job(command, tag = "rivet_htcondor_job"):
  ...
  def send_job(self, command, tag="rivet_htcondor_job"):
    # make files
    self._tag = tag
  ...
  def _make_job_file(self, runFile) :
    batchFile = os.path.join(self.batch_path, "batch_rivet__{0}.job".format(self._tag))
